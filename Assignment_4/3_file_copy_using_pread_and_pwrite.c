...
BOOL copyData(int srcFileFd, int dstFileFd)
{
	...
	int iRet = 0, srcFileReadOffset = 0, dstFileWriteOffset = 0, writeRet = 0;

	errno = 0;
	while((iRet = pread(srcFileFd, buffer, BLOCK_SIZE, srcFileReadOffset)) != 0)
	{
		if(errno)
		{
		...
		srcFileReadOffset += iRet;

		writeRet = pwrite(dstFileFd, buffer, iRet, dstFileWriteOffset);
